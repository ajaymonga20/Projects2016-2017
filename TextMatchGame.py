...
class TextMatchGame:
	...
	def run(self):
		...
		scoretext = font.render("Score: " + str(self.score), True, (250, 250, 250))
		scoreRect = scoretext.get_rect()
		scoreRect.x = 600
		scoreRect.y = 100	
		text = font.render("Press g to restart!", True, (250,250,250))	
		...
		bird.transform = pygame.transform.scale(bird.image, (40,40))
		...
		while keeprunning:	
			...
			#while Gtk.events_pending():
				#Gtk.main_iteration()	
			scoretext = font.render("Flaps: " + str(self.score), True, (250, 250, 250))
			text = font.render("Press g to restart!", True, (250,250,250))
			pygame.mixer.init()
			hit = pygame.mixer.Sound("blip.wav")
			self.clock.tick(60)	
			#gameDisplay.blit(opener,(0,0))
			#gameDisplay.blit(button,imgRect)
			pygame.display.update()
			for event in pygame.event.get():
				if event.type == pygame.QUIT:
					...
					if pygame.mouse.get_pos()[0] > 550 and pygame.mouse.get_pos()[1] > 600:
						...
						#pygame.display	
					elif pygame.mouse.get_pos()[0] > 1100 and pygame.mouse.get_pos()[1] < 70:
						self.score = 0
						self.birdY = 625
						self.birdX = 30	
						...
						if event.key == pygame.K_LEFT:
							...
							if self.birdX >= 761 and self.birdX <= 1010 and self.birdY > 570:
								...
								self.birdX += 5
								self.birdY = 569	
							if self.birdX >= 0 and self.birdX <= 1200:
								if self.birdX >= 761 and self.birdX <= 1012:
									self.birdX += 30
								else:
									self.birdX *= 2.4
			...
			if self.birdX >= 761 and self.birdX <= 1012 and self.birdY > 570:
				self.speed = 0
				if self.birdX >= 945 and self.birdX <= 970:
					flg = False 
					gameDisplay.fill((255,0,0))
					gameDisplay.blit(scoretext,scoreRect)
					gameDisplay.blit(text,textRect)
					if self.score <= 6:
						gameDisplay.blit(gold,(500,450))
					if self.score == 7:
						gameDisplay.blit(silver,(500,450))
					if self.score ==8:
						gameDisplay.blit(bronze,(500,450))	
